    ...
    <div className="pt-14">
      ...
      <div>
        {orderData && orderData.length > 0 ? (
          <>
            {orderData.map((order) => (
              <div key={order._id} className="py-4 border-t border-b text-gray-700 flex flex-col md:flex-row md:items-center md:justify-around gap-4">
                <div className="flex flex-col gap-4 w-full">
                  {order.products.map((prod, index) => {
                    const productDetails = products.find((p) => p._id === prod.productId);
                    return (
                      <div key={index} className="flex items-center gap-6 text-sm">
                        {productDetails && (
                          <img
                            src={Array.isArray(productDetails.image) ? productDetails.image[0] : productDetails.image}
                  ...
                  <p className="mt-2">
                    Date: <span className="text-gray-400">{order.date ? new Date(order.date).toLocaleDateString() : "Not available"}</span>
                  </p>
            ...
            {/* One single button for all orders */}
